...
import { joinPartyRoom } from "@/lib/party/partyLifecycleService";
...
export const POST = async (request: Request, context: { params: Promise<{ code: string }> }) => {
    ...
    if (await consumeWriteRateLimit(gate.user.id, "party-membership", 60, 900)) {
        return NextResponse.json({ error: "Zbyt wiele prób dołączenia do pokoju." }, { status: 429 });
    }
    ...
    try {
        const result = await joinPartyRoom(gate.user, code);
